...
if __name__ == '__main__':
    ...
    train_sample = train_annotations.sample(frac=0.001, random_state=42)
    test_sample = test_annotations.sample(frac=0.001, random_state=42)
    ...
    with torch.no_grad():
        ...
        for images, targets in test_loader:
            ...
            # Collecting valid predictions and their corresponding true labels
            valid_preds_scores = []
            valid_true_labels = []

            for output, target in zip(outputs, targets):
                if len(output['scores']) > 0:
                    valid_preds_scores.append(output['scores'])
                    valid_true_labels.append(target['labels'])

            if valid_preds_scores:
                preds_scores = torch.cat(valid_preds_scores).to(device)
                true_labels = torch.cat(valid_true_labels).to(device)
