...
def compute_map_train(detection_results, ground_truths):
...
def compute_map_eval(detection_results, ground_truths):
    mean_ap_metric = MeanAveragePrecision(iou_thresholds=[0.5], class_metrics=True)
    mean_ap_metric.update(detection_results, ground_truths)
    mAP = mean_ap_metric.compute()
    ...
    if map_per_class.nelement() > 1 and map_per_class[0].item() != -1:
        for i, ap in enumerate(map_per_class):
            if ap.item() == -1:
                print(f"Class {i} has no positive samples.")
            else:
                print(f"Average Precision for class {i}: {ap.item():.4f}")
    else:
...
if __name__ == '__main__':
    ...
    train_sample = train_annotations.sample(frac=0.002, random_state=42)
    test_sample = test_annotations.sample(frac=0.002, random_state=42)
    ...
    train_loader = DataLoader(
        train_dataset,
        batch_size=1, #Decreased for testing purpose on my laptop (It was 1, but now i put it back at 5 in order to run it on the cluster)
        shuffle=True,
    ...
    for epoch in range(num_epochs):
        ...
        ground_truths_1, detection_results_train = evaluate(model, train_loader, device)
        ground_truths_2, detection_results_test = evaluate(model, test_loader, device)
        val_loss = test_one_epoch(model, test_loader, device, epoch)
        val_losses.append(val_loss)
        mAP_train = compute_map_train(detection_results_train, ground_truths_1)
        mAP_train_values.append(mAP_train)
        mAP_test = compute_map_train(detection_results_test, ground_truths_2)
        mAP_val_values.append(mAP_test)
    
    # evaluate and compute AP for each class
    ground_truths, detection_results = evaluate(model, test_loader, device)
    num_classes = 19  
    map_metrics = compute_map_eval(detection_results, ground_truths)
